...
```{r}
top_5_status <- sfo_trees_df |>
  drop_na(legal_status)filter(!is.na(legal_status)) |>
  ### filter(!is.na(legal_status)) |>### same thing!
  group_by(legal_status) |>
  summarize(tree_count = n()) |>
  ungroup() |>
  slice_max(tree_count, n = 5) |>
...
get rid of all columns not what we want #filter()
filter out plants that are not pine or oak
summarize the counts of the oak and pines
